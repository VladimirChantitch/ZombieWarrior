...
public class DragAndDropManipulator : PointerManipulator
{
    ...
    private void PointerMoveHandler(PointerMoveEvent evt)
    {
        if (enabled && target.HasPointerCapture(evt.pointerId))
        {
            ...
            target.transform.position = new Vector2(
                Mathf.Clamp(targetStartPosition.x + pointerDelta.x, -15, target.panel.visualTree.worldBound.width - targetStartPosition.x- pointerDelta.x),
                Mathf.Clamp(targetStartPosition.y + pointerDelta.y, -200, target.panel.visualTree.worldBound.height));
        }
    }
    ...
    private void PointerCaptureOutHandler(PointerCaptureOutEvent evt)
    {
        if (enabled)
        {
            VisualElement slotsContainer = root.Q<VisualElement>("PrincipalLayout");
            UQueryBuilder<VisualElement> allSlots =
                slotsContainer.Query<VisualElement>(className: "style-slot");
